// TODO: Split into feature service modules (user.service, group.service, expense.service, settlement.service, balance.service, history.service, settings.service, ticket.service)
// This is the unified Firestore service layer.
...
export async function getGroupsByUserId(userId: string): Promise<Group[]> {
    ...
    const groupDocs = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as GroupDocument & { id: string }));
    groupDocs.forEach(g => {
