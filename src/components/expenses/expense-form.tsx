...
export function ExpenseForm({ group }: ExpenseFormProps) {
  ...
  // Safe access for category logic
  ...
  // Ensure Icons is treated as an indexable object
  const CategoryIcon = (Icons as any)[categoryIconName] || Icons.Wallet;
      ...
      <FormField
        ...
        render={({ field }) => (
          <FormItem>
            ...
            <FormControl>
              <Input placeholder="e.g., Dinner at BBQ Nation" {...field} />
            </FormControl>
            <FormMessage />
          </FormItem>
        )}
            ...
            <FormControl>
              ...
              >
                <TabsList className="grid w-full grid-cols-2">
                  ...
                  <TabsTrigger value="multiple">Multiple People</TabsTrigger>
                </TabsList>
              </Tabs>
            </FormControl>
            <FormMessage />
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex items-center gap-2 space-y-0">
                  <FormLabel className="font-normal w-1/2">
                    {watch(`multiPayers.${index}.name`)}
                  </FormLabel>
      ...
      <div className="space-y-2">
        ...
        <ScrollArea className="h-40 border rounded-md">
          <div className="p-2 space-y-1">
            {watchParticipants &&
              watchParticipants.map((p: any, index: number) => (
                <div
                  key={p.userId}
                  className={cn('flex items-center gap-3 p-2 rounded-md', p.selected ? 'bg-muted/50' : '')}
                >
                  <FormField
                    control={control}
                  ...
                  {p.selected && (
                    <>
                      {watchSplitType === 'equally' && (
                        <p className="text-sm text-right text-muted-foreground w-24">
                          {CURRENCY_SYMBOL}
